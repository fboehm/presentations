...
main_findings:
  - "Allelic series informs QTL mapping in a second cohort"
logocenter_name: flowcode.png
output: 
...
1. Use CC QTL allelic series to map QTL in DO mice

# Methods

- We measured *M. tuberculosis* burden in lungs and spleen in a DO cohort (763 mice) and a CC cohort (53 lines)
    - We also measured cytokine levels in the CC cohort
...
1. Traditional QTL mapping in CC identified 40 QTL peaks for 11 traits
1. For each CC QTL, we:
    ...
    1. identified any proximal DO QTL with LOD greater than 2
...
t4$cis_peaks[[36]] %>%
  tibble::as_tibble() %>%
  dplyr::mutate(chr = as.numeric(as.character(chr))) %>%
  dplyr::bind_rows(t4[36, 1:5]) %>%
  dplyr::mutate(cohort = c("DO", "DO", "CC")) %>%
  dplyr::mutate(trait = c("Peak weight in DO", "Euthanasia weight in DO", "VEGF in CC")) %>%
  dplyr::select(-1, -2) %>%
  dplyr::select(trait, cohort, chr, pos, lod) %>%
  dplyr::arrange(pos) %>% 
  gt::gt() %>%
  gt::tab_header(title = "QTL peaks on Chr 3") %>%
...
1. Use multiple allelic series per CC QTL  

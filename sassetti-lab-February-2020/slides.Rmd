...
if (!file.exists(afn)){
  ...
  saveRDS(kinship, "../data/kinship.rds")
} else {
  aprobs <- readRDS(afn)
...
1. Map QTL in DO after identifying allelic series for QTL in CC
1. 
...
```{r, child="~/work/research/qtl2tb/Rmd/fda_reducedscan.Rmd", eval = FALSE}
```
...
1. Allelic series inferences and QTL mapping with simulated traits
  a. Quantify QTL mapping power of our approach
...
1. Simulate traits in both cohorts (CC and DO)
  a. Vary allelic series
    - number of alleles and configurations
